...
enum Pince_V_Action{Pince_V_UP,Pince_V_DOWN,Pince_V_Bourrage,Pince_V_Pousse,Pince_V_droite_up,Pince_V_gauche_up};
...
Etat_Robot Robot_Principal=Prechauff;
 ...
 struct Consigne {
  ...
  bool Rampe_Distance_Active=true;
  ...
  Consigne(float X,float Y,float A,int32_t Time,bool Rampe_Active, Consigne *Next ):Action( Deplacement),X_DEST(X),Y_DEST(Y),ANGLE_FINAL(A),TimeOut(Time),consigne_suivante(Next),Rampe_Distance_Active(Rampe_Active) {}
